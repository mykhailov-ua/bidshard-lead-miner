...
# Cron scrape pipes NDJSON on stdout; IPC socket is realtime-only.
set_env TELETHON_IPC_SOCKET ''
set_env TELETHON_IPC_FORMAT ndjson
set_env TELEGRAM_SESSION_ROLE hot
set_env TELEGRAM_LEASE_ENABLED 1
set_env TELEGRAM_LEASE_TTL_SEC 300
set_env TELEGRAM_LEASE_MAX_CHATS 40
# Stagger cron: min 40m between runs per session; up to 2m random delay.
set_env TELEGRAM_CRON_MIN_GAP_SEC 2400
set_env TELEGRAM_CRON_JITTER_SEC 120
...
set_env TELEGRAM_GLOBAL_SEARCH_LIMIT 6
set_env TELEGRAM_GLOBAL_SEARCH_DAILY_LIMIT 24
set_env TELEGRAM_GLOBAL_SEARCH_UTC_HOURS 0-23
set_env TELEGRAM_INVITE_JOIN 1
set_env TELEGRAM_INVITE_JOIN_HOT 1
set_env TELEGRAM_INVITE_JOIN_LIMIT 5
set_env TELEGRAM_USER_ENRICH_LIMIT 40
set_env TELEGRAM_USER_ENRICH_PREFILTER_ONLY 0
set_env TELEGRAM_SCRAPE_CROSS_MENTION 1
set_env TELEGRAM_PARTICIPANT_PROFILE_LIMIT 25
set_env TELEGRAM_PROFILE_LINK_ENQUEUE 1
set_env TELEGRAM_PROFILE_LINK_ENQUEUE_LIMIT 50
set_env TELEGRAM_PROFILE_LINK_MIN_FIT maybe
set_env TELEGRAM_PROFILE_LINK_DRAIN 1
set_env TELEGRAM_PROFILE_LINK_DRAIN_LIMIT 20
...
set_env CRM_TELEGRAM_LEAD_NOTIFY_MIN_SCORE 30
set_env CRM_TELEGRAM_LEAD_NOTIFY_MIN_SCORE_NON_TELEGRAM 70
set_env PARSER_ACCEPT_MIN_SCORE 70
set_env PARSER_TELEGRAM_ACCEPT_MIN_SCORE 30
# LEADS.md Phase 0: locale overlays for LATAM/SEA scoring (es, pt, id, vi).
set_env KEYWORDS_LOCALE 'es,pt,id,vi'
...
set_env PARSER_SERP_DORK_BATCH 24
